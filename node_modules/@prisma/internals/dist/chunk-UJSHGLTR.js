...
var chunk_UJSHGLTR_exports = {};
__export(chunk_UJSHGLTR_exports, {
...
module.exports = __toCommonJS(chunk_UJSHGLTR_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_XQMKDIJQ.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_NZEQODI5.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_NZEQODI5.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_XQMKDIJQ.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_M4YKH3QB.getConfig)({ datamodel: schemaResult.schemas });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_6XDXS7TR.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_MAEEGX2B.mergeSchemas)({ schemas: schemaContext.schemaFiles });
        const options2 = {
          datamodel,
